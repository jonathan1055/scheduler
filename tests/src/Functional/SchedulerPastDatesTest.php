...
class SchedulerPastDatesTest extends SchedulerBrowserTestBase {
  ...
  public function testSchedulerPastDates() {
    ...
    $this->assertSession()->pageTextContains("The 'publish on' date must be in the future", 'An error message is shown by default when the publication date is in the past.');

    // Test the 'error' behavior explicitly.
    $this->nodetype->setThirdPartySetting('scheduler', 'publish_past_date', 'error')->save();
    $this->drupalPostForm('node/' . $node->id() . '/edit', $edit, 'Save');
    $this->assertSession()->pageTextContains("The 'publish on' date must be in the future", 'An error message is shown when the publication date is in the past and the "error" behavior is chosen.');
    ...
    $this->assertSession()->pageTextContains(sprintf('%s %s has been updated.', $this->typeName, $edit['title[0][value]']), 'The node is saved successfully when the publication date is in the past and the "publish" behavior is chosen.');
    ...
    $this->assertSession()->pageTextContains(sprintf('%s %s has been updated.', $this->typeName, $edit['title[0][value]']), 'The node is saved successfully when the publication date is in the past and the "schedule" behavior is chosen.');
    $this->assertSession()->pageTextContains('will be published', 'The node is scheduled to be published when the publication date is in the past and the "schedule" behavior is chosen.');
    ...
    $this->assertSession()->pageTextContains("The 'unpublish on' date must be in the future", 'An error message is shown when the unpublish date is in the past.');
  }
